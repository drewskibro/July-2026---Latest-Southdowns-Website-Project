<?php
/**
 * Shared travel-destination page body (Kenya / Thailand / India / Cape Verde / Africa).
 *
 * Included by the thin page-templates/page-*-travel-vaccines.php wrappers, which
 * set $tv_tpl before including:
 *   'hero_image'     Default hero image URL   (ACF tv_hero_image overrides)
 *   'hero_alt'       Hero image alt text
 *   'spotlight_image' Default intro-spotlight image (ACF tv_spotlight_image overrides;
 *                     falls back to hero_image)
 *   'spotlight_alt'  Spotlight image alt text
 *   'embed_calendar' true = render the on-page Amelia travel calendar (#book) and
 *                    point Book CTAs at it; false = CTAs go to /book-appointment/.
 *
 * All copy comes from tv_data() defaults, overridable per page via the
 * Travel Vaccines ACF field group.
 *
 * @package Southdowns_Pharmacy
 */

$tv_tpl = wp_parse_args( $tv_tpl ?? [], [
	'hero_image'      => 'https://images.unsplash.com/photo-1516026672322-bc52d61a55d5?w=1200&q=80&auto=format&fit=crop',
	'hero_alt'        => 'Travel destination landscape',
	'spotlight_image' => '',
	'spotlight_alt'   => '',
...
if ( '' === $tv_tpl['spotlight_image'] ) {
	...
	$tv_tpl['spotlight_alt']   = $tv_tpl['spotlight_alt'] ?: $tv_tpl['hero_alt'];
}
...
<section class="relative w-full min-h-[500px] lg:min-h-[600px] overflow-hidden">
  ...
  <div class="hidden lg:grid grid-cols-2 min-h-[600px]">
    ...
    <div class="relative flex items-center px-12 xl:px-20 py-20" style="background:#1a73e9;">
      ...
      <div class="relative z-10 max-w-xl">
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-8">
          <a href="<?php echo esc_url( $tv_book_href ); ?>" class="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-bold px-6 py-3.5 rounded-xl font-jost hover:bg-blue-50 transition-colors duration-200 shadow-lg">
    ...
    <!-- Right: image -->
    <div class="relative">
      <img src="<?php echo esc_url( sp_field( 'tv_hero_image', $tv_tpl['hero_image'] ) ); ?>" alt="<?php echo esc_attr( $tv_tpl['hero_alt'] ); ?>" class="absolute inset-0 w-full h-full object-cover" loading="eager"/>
  ...
  <div class="lg:hidden relative min-h-[500px]" style="background:#1a73e9;">
    <img src="<?php echo esc_url( sp_field( 'tv_hero_image', $tv_tpl['hero_image'] ) ); ?>" alt="<?php echo esc_attr( $tv_tpl['hero_alt'] ); ?>" class="absolute inset-0 w-full h-full object-cover opacity-30" loading="eager"/>
    ...
    <div class="relative z-10 px-6 py-16 sm:px-10 max-w-2xl">
      ...
      <div class="flex flex-col gap-3 mb-6">
        <a href="<?php echo esc_url( $tv_book_href ); ?>" class="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-bold px-5 py-3 rounded-xl font-jost hover:bg-blue-50 transition-colors duration-200 shadow-lg">
          <?php echo sp_field( 'tv_hero_btn1', $tv['tv_hero_btn1'] ); ?>
        </a>
...
<!-- ═══════════════════════════════════════════════════════
     S1.5 · TRUST STATS BAND (gradient)
════════════════════════════════════════════════════════ -->
<section class="py-10 md:py-12" style="background: linear-gradient(135deg,#1e3a8a 0%,#1d4ed8 50%,#3b82f6 100%);">
  <div class="section-container">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
      <?php
      $tv_band_stats = sp_rows( 'tv_trust', $tv['tv_trust'], [ 'value' => 'value', 'label' => 'label' ] );
      foreach ( $tv_band_stats as $band_stat ) : ?>
      <div class="text-center">
        <div class="text-2xl md:text-4xl font-bold text-white mb-1 font-jost"><?php echo esc_html( $band_stat['value'] ); ?></div>
        <div class="text-blue-100 text-xs md:text-sm font-jost"><?php echo esc_html( $band_stat['label'] ); ?></div>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
...
<!-- ═══════════════════════════════════════════════════════
     S2 · INTRO SPOTLIGHT  (light, copy + lifestyle image)
════════════════════════════════════════════════════════ -->
  ...
  <div class="section-container">
    <div class="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
      <div>
        ...
        <h2 class="text-3xl md:text-4xl lg:text-[42px] font-bold text-slate-900 mb-5 font-jost leading-tight"><?php echo sp_field( 'tv_intro_h2', $tv['tv_intro_h2'] ); ?></h2>
        <p class="text-slate-700 text-lg mb-5 font-jost leading-relaxed"><?php echo sp_field( 'tv_intro_p1', $tv['tv_intro_p1'] ); ?></p>
        <p class="text-slate-600 text-base font-jost leading-relaxed mb-8"><?php echo sp_field( 'tv_intro_p2', $tv['tv_intro_p2'] ); ?></p>
        <div class="flex flex-wrap gap-x-6 gap-y-3 mb-8">
          <?php foreach ( [ 'GPhC-Registered Pharmacists', 'Same-Day Appointments', 'No GP Referral Needed' ] as $intro_point ) : ?>
          <div class="flex items-center gap-2 text-slate-700 font-jost text-sm md:text-base font-medium">
            ...
            <?php echo esc_html( $intro_point ); ?>
          </div>
          <?php endforeach; ?>
        </div>
        <a href="<?php echo esc_url( $tv_book_href ); ?>" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-7 py-3.5 rounded-full transition-colors shadow-lg shadow-blue-500/20 font-jost">
          <?php echo sp_field( 'tv_hero_btn1', $tv['tv_hero_btn1'] ); ?>
      ...
      <div class="relative">
        <div class="rounded-3xl overflow-hidden aspect-[4/3] shadow-2xl shadow-blue-900/15">
          <img src="<?php echo esc_url( sp_field( 'tv_spotlight_image', $tv_tpl['spotlight_image'] ) ); ?>" alt="<?php echo esc_attr( $tv_tpl['spotlight_alt'] ); ?>" class="w-full h-full object-cover" loading="lazy" />
        </div>
        <div class="absolute -bottom-5 -left-5 md:-bottom-6 md:-left-6 bg-white rounded-2xl shadow-xl border border-slate-100 px-5 py-4 flex items-center gap-3">
          ...
          <div>
            <p class="text-slate-900 font-bold text-sm font-jost leading-tight">Same-Day Appointments</p>
            <p class="text-slate-500 text-xs font-jost">Walk-ins welcome at all branches</p>
          </div>
        </div>
      </div>
    </div>
...
════════════════════════════════════════════════════════ -->
<section id="vaccines" class="py-20 md:py-28 bg-gradient-to-b from-slate-50 to-white">
  <div class="section-container">
    <div class="text-center mb-14">
      ...
      <h2 class="text-3xl md:text-4xl font-bold text-slate-900 mb-4 font-jost"><?php echo sp_field( 'tv_vax_h2', $tv['tv_vax_h2'] ); ?></h2>
      <p class="text-slate-600 text-lg max-w-2xl mx-auto font-jost"><?php echo sp_field( 'tv_vax_intro', $tv['tv_vax_intro'] ); ?></p>
    </div>
    ...
    <div class="space-y-12 max-w-5xl mx-auto">
      ...
      <div>
        <!-- Group header -->
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-6 border-b border-slate-200 pb-4">
          ...
          <p class="text-slate-500 text-sm font-jost"><?php echo esc_html( $meta['subhead'] ); ?></p>
        </div>

        <!-- Vaccine cards -->
        <div class="grid md:grid-cols-2 gap-4">
          <?php foreach ( $glist as $v ) : $vnum++; ?>
          <div class="bg-white border border-slate-200 rounded-2xl p-6 md:p-7 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 reveal-item">
            <div class="flex items-start gap-4 mb-4">
              <span class="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center text-sm font-bold font-jost <?php echo esc_attr( $bullet_styles[ $meta['colour'] ] ); ?>">
                <?php echo esc_html( str_pad( (string) $vnum, 2, '0', STR_PAD_LEFT ) ); ?>
              </span>
              <h3 class="text-xl font-bold text-slate-900 font-jost leading-snug pt-1.5"><?php echo wp_kses_post( $v['name'] ); ?></h3>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              <div class="bg-slate-50 rounded-xl px-4 py-3">
                <p class="text-slate-400 text-xs font-semibold uppercase tracking-wider font-jost mb-1">Who needs it</p>
                <p class="text-slate-700 text-[15px] font-jost font-medium"><?php echo esc_html( $v['who'] ); ?></p>
              </div>
              <div class="bg-slate-50 rounded-xl px-4 py-3">
                <p class="text-slate-400 text-xs font-semibold uppercase tracking-wider font-jost mb-1">Doses</p>
                <p class="text-slate-700 text-[15px] font-jost font-medium"><?php echo wp_kses_post( $v['doses'] ); ?></p>
              </div>
            </div>
            <p class="text-slate-600 text-[15px] font-jost leading-relaxed"><?php echo wp_kses_post( $v['desc'] ); ?></p>
            ...
            <div class="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
              <div class="flex items-start gap-3">
                <svg class="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                <div>
                  <p class="text-amber-900 text-sm font-jost font-semibold leading-relaxed">Yellow Fever vaccinations and ICVP certificates are available at our Bosmere Pharmacy, Havant location only.</p>
  ...
  <div class="section-container">
    ...
    <div class="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto">
      ...
      <div class="bg-white rounded-2xl p-7 md:p-8 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 reveal-item border-t-4 <?php echo esc_attr( $ra['top'] ); ?>">
        ...
        <p class="text-slate-600 text-[15px] font-jost leading-relaxed"><?php echo wp_kses_post( $r['desc'] ); ?></p>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
      ...
      <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg transition-shadow duration-300 reveal-item flex flex-col">
        ...
        <div class="p-5 space-y-2.5 flex flex-col flex-1">
          ...
          <div class="mt-auto pt-2 flex items-center justify-between gap-3">
            <a href="<?php echo esc_url( sp_branch_url( $i ) ); ?>" class="text-slate-500 text-sm font-semibold font-jost hover:text-blue-600 transition-colors">View <?php echo esc_html( $b['name'] ); ?></a>
            <a href="<?php echo esc_url( $tv_book_href ); ?>" class="inline-flex items-center gap-1.5 text-blue-600 font-semibold text-sm font-jost hover:text-blue-700 transition-colors duration-200">
              Book here
              <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </a>
          </div>
...
<?php if ( $tv_tpl['embed_calendar'] ) : ?>
<!-- ═══════════════════════════════════════════════════════
     S11.5 · BOOK ONLINE — embedded travel clinic calendar
     (rendered only when the wrapper sets embed_calendar = true)
     Amelia category 9 (Travel Health), location selectable so Amelia
     enforces which branches offer travel vaccines. Yellow Fever is routed
     separately to the dedicated Bosmere calendar at /yellow-fever/#book.
════════════════════════════════════════════════════════ -->
<section id="book" class="py-20 md:py-28 bg-[#fdf9f6] border-t border-[#e8e0d8]">
  <div class="section-container">
    <div class="text-center mb-8 md:mb-10">
      ...
      <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-slate-800 mb-6 font-jost">Book Your Travel Consultation</h2>
      <p class="text-lg md:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed font-jost">Choose your branch, service and time below. <strong>Yellow Fever is available at Bosmere, Havant only</strong> &mdash; <a href="<?php echo esc_url( home_url( '/yellow-fever/' ) . '#book' ); ?>" class="text-blue-700 font-semibold underline hover:text-blue-800">book your Yellow Fever appointment here</a>.</p>
    </div>
...
<!-- ═══════════════════════════════════════════════════════
     S12 · FINAL CTA  (dark, id="book-cta")
════════════════════════════════════════════════════════ -->
<section id="book-cta" class="py-20 md:py-28 relative overflow-hidden" style="background: linear-gradient(160deg,#0f172a 0%,#1e3a8a 55%,#1d4ed8 100%);">
  <div class="absolute inset-0 dot-pattern pointer-events-none"></div>
  ...
  <div class="section-container relative z-10 text-center">
    ...
    <div class="flex flex-col sm:flex-row gap-4 justify-center mb-16">
      <a href="<?php echo esc_url( $tv_book_href ); ?>" class="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-bold px-8 py-4 rounded-xl font-jost hover:bg-blue-50 transition-colors duration-200 shadow-xl shadow-blue-900/30 text-lg">
        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
